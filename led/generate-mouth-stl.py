...
def basehalf():
    ...
    for i in range(2): #16
        for j in range(8):
            circle = Circle((Point(2+4*i,2+4*j,0), Z), hole_size)
    ...
    return model + model.transform(8*X) + model.transform(16*X) + model.transform(24*X)
...
m = base + base.transform(32*X) + wall
