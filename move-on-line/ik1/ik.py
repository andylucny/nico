...
angles_from = torch.tensor([-30.0,-30.0,0.0,50.0,-180.0,-180.0,-180.0],dtype=torch.float32).to(device)
angles_to = torch.tensor([80.0,180.0,70.0,180.0,180.0,180.0,180.0],dtype=torch.float32).to(device)

goal_point = closest_point_on_line(start_point, touch_point, point).to(device)

angles = torch.tensor(initial_angles, requires_grad=True).to(device)
initial_logits = [ angle.item() for angle in sigmoid_inv(torch.tensor(list((initial_angles-angles_from)/(angles_to-angles_from)))) ]
logits = torch.tensor(initial_logits, requires_grad=True).to(device)
...
def train(epoches=60000):
    ...
    for step in range(epoches):  # Number of iterations
        ...
        angles = angles_from + sigmoid(logits) * (angles_to-angles_from)
        ...
        with torch.no_grad():
            pom = learning_rate * logits.grad
            angles_pom = angles_from + sigmoid(logits - pom) * (angles_to-angles_from) 
            if ((10*(angles-angles_pom)).round() - (10*angles).round()).abs().sum() == 0:
                same += 1
            else:
                same = 0
            logits -= pom
...
def improve(start_point_, touch_point_, initial_angles_, point_=None, epoches=60000, device='cpu'):
    global start_point, touch_point, initial_angles, goal_vector, goal_point, angles, logits
