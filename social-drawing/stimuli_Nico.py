...
import json
import ndjson
from nico.main import startup, setmode, shutdown, start_video, stop_video, enableTorque, disableTorque
...
categories = [tr('Sheep','Ovca'), tr('Bus','Autobus'), tr('Bee','Včela'),
              ...
              tr('Mosquito','Komár'), tr('Map','Mapa'), tr('Sea Turtle','Morská korytnačka'),
              tr('Pizza','Pizza'), tr('Teddy Bear','Plyšový medvedík'), tr('Pig','Prasiatko')]

categories_eng =  ['Sheep', 'Bus', 'Bee','Face', 'Computer', 'Duck',
...
def drawing_questions(n):
    ...
    slider = visual.Slider(win, ticks=(0, 100), labels=(0, 7), granularity=0.1, pos=(0, -100), size=(1000, 50), font='Helvetica',
                           units='pix')
    ...
    text = visual.TextStim(win, text=tr("How much did you enjoy drawing the ", "Ako veľmi sa Vám páčilo kresliť: ") + categories[n] + "? \n" +
                            tr("(1 - not enjoyed, 7 - extremely enjoyed)", "(1 - vôbec sa mi nepáčilo, 7 - veľmi sa mi páčilo)"), font='Helvetica', 
    ...
    text = visual.TextStim(win, text=tr("How much do you like your drawing of the ","Ako veľmi sa Vám páči Váš výkres: ") + categories[n] + "? \n" +
                           tr("(1 - not liked, 7 - liked a lot)", "(1 - vôbec sa mi nepáči, 7 - veľmi sa mi páči)"), color=(1, 1, 1), font='Helvetica', 
...
def drawing_activity(i):
    ...
    p = subprocess.Popen(
        ["C:/Experiment/Drawing/drawing_venv/Scripts/python", script_path, categories_eng[i], path_folder_participant,
...
def artistic_questions():
    ...
    text = visual.TextStim(win,
                           text=tr("How much do you enjoy free-hand drawing? \n (1 - extremely little, 7 - extremely much) \n","Ako veľmi sa Vám páčilo kreslenie prstom? \n (1 - vôbec sa mi nepáčilo, 7 - veľmi sa mi páčilo) \n") +
                           tr("(click on the slider and then drag the marker)","(Kliknite na posuvník a potom potiahnite značku)"),
                           color=(1, 1, 1), pos=(0.0, 11.0), colorSpace='rgb', bold=False, height=2.5, font='Helvetica', anchorHoriz="center", wrapWidth=400)
    text.draw()
    ...
    text = visual.TextStim(win, text=tr("How often do you draw sketches? \n (1 - extremely little, 7 - extremely much)", "Ako často kreslíte? \n (1 - veľmi zriedkavo, 7 - veľmi často)"),
                           color=(1, 1, 1), pos=(0.0, 11.0), colorSpace='rgb', font='Helvetica', bold=False, height=2.5, anchorHoriz="center", wrapWidth=400)
    text.draw()
...
def main():
    ...
    text = visual.TextStim(win, text=tr("Welcome!\nThis is a first trial to help\n", "Vitajte! \nToto je prvá skúška, ktorá Vám pomôže\n") +
                                     tr("you understand how the drawing activity will work.", "porozumieť ako bude aktivita prebiehať"), color=(1, 1, 1), pos=(0.0, 11.0),
    ...
    text = visual.TextStim(win, text=tr("Great!\n Is everything clear? Then you can\n", "Výborne!\n Je všetko jasné? Ak áno, môžeme pokračovať\n") +
                                     tr("proceed with the actual experiment.", "s experimentom."), color=(1, 1, 1), pos=(0.0, 11.0), colorSpace='rgb',
                           bold=False, height=2.5, font='Helvetica', anchorHoriz="center", wrapWidth=400)
    text.draw()
    ...
    text = visual.TextStim(win, text=tr("Welcome!\nWe will ask you to draw several pictures\n", "Vitajte!\n Požiadame Vás, aby ste nakreslili pár obrázkov\n") +
                                     ...
                                     tr("Are you ready? ", "Ste pripravení? "), color=(1, 1, 1), pos=(0.0, 11.0),
                           colorSpace='rgb', bold=False, height=2.5, font='Helvetica', anchorHoriz="center", wrapWidth=400)
    text.draw()
